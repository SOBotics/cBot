...
void *commandThread(void *arg) {
    ...
    for (int i = 1; commandList[i]; i ++) {
        char *name = malloc(strlen(commandList[i]->name) + 1);
        strcpy(name, commandList[i]->name);
        char *name_orig = name; //This will allow us to free it.
        ...
        for(j = 0; (token = strsep(&name, " ")); j++) {
            if(strcmp("...", token) == 0) {
                //This means that everything else is arguments; add them to the argument list.
